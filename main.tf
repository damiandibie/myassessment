...
# Route 53 P Hosted Zone
resource "aws_route53_zone" "public1" {
  provider = aws.singapore
  name     = "damiandibie.xyz"


}


resource "aws_route53_record" "dam-sg-rou53" {
  provider = aws.singapore
  
  zone_id = aws_route53_zone.public1.zone_id
  name    = "app"  
  type    = "A"
  ...
  alias {
    name                  = aws_lb.dam-sg-alb.dns_name
    zone_id               = aws_lb.dam-sg-alb.zone_id
    evaluate_target_health = true
  }
  set_identifier = "singapore"
  
}
resource "aws_route53_record" "dam-ie-rou53" {
  provider = aws.ireland
  ...
  weighted_routing_policy {
    ...
    alias {
    name                  = aws_lb.dam-ie-alb.dns_name
    zone_id               = aws_lb.dam-ie-alb.zone_id
    evaluate_target_health = true
  }
